...
function GetHash(auth_date, query_id, signature, user) {
    const secret = crypto
...
function verifyTelegramWebAppData(telegramInitData) {
    try {
        ...
        return Math.round(new Date().getTime() / 1000) - parseInt(telegramInitData.auth_date) < 3600 * 24
            && GetHash(telegramInitData.auth_date, telegramInitData.query_id, telegramInitData.signature, telegramInitData.user) == hash;
    }
    catch (e) {
        console.error(e);
        return false;
